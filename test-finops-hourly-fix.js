...
// Test dates - using a past date that should have data
const testDates = [
      ...
      res.on('end', () => {
        try {
          ...
          if (rows.length === 0) {
            ...
            resolve({ fromDate, toDate, count: 0, rows: [] });
            return;
          }
          ...
          resolve({ fromDate, toDate, count: rows.length, hoursWithData, missingHours, rows });
        } catch (e) {
...
async function runTests() {
  ...
  for (const date of testDates) {
    try {
      ...
      console.error(`Failed to test ${date}:`, e.message);
    }
  }
  ...
  results.forEach(result => {
    ...
    if (result.missingHours && result.missingHours.length > 0) {
  ...
  if (hasIssues) {
    console.log('\n❌ ISSUES DETECTED: Some hours are missing data on past dates');
    process.exit(1);
  } else {

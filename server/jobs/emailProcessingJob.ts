...
import { getAllActiveConfigs, processEmailsForConfigs, getTodayEmails } from "../services/emailProcessorService";
  ...
  try {
    ...
    cron.schedule("*/5 * * * *", async () => {
      ...
      try {
        ...
        console.error("Error running email processing job:", (err as any)?.message || err);
      }
    }, {
      scheduled: true,
    });
    ...
    console.error("Failed to initialize email processing job:", (error as any)?.message || error);
  }
}

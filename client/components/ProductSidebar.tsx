    ...
    <div className="p-4">
      <div className="mb-4">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2 text-xs">
          <div className="p-2 bg-white rounded shadow">Total<br/>{counts.total}</div>
          <div className="p-2 bg-white rounded shadow">Pending<br/>{counts.pending}</div>
          <div className="p-2 bg-white rounded shadow">In Progress<br/>{counts.inprogress}</div>
          <div className="p-2 bg-white rounded shadow">Completed<br/>{counts.completed}</div>
        </div>
      </div>
            ...
            <Card key={p.id} className="p-2">
              <CardContent className="p-2 flex justify-between items-start">
                <div>
                  ...
                  <div className="text-xs">{p.current_version} • {p.status}</div>
                </div>
                <div className="flex flex-col gap-2">
                  <Button variant="ghost" onClick={()=>navigate(`/products/${p.id}`)}>View</Button>
                </div>
              </CardContent>
            </Card>
          ))}

...
function ApprovalTimer({ taskId, subtaskId }: { taskId: number; subtaskId: number }) {
  ...
  React.useEffect(() => {
    ...
    async function fetchNext() {
      try {
        const res = await apiClient.get(`/finops-production/external-alerts?task_id=${taskId}&subtask_id=${subtaskId}`);
  ...
  return (
    <div className="text-sm text-gray-600 mt-2">
      <div>
        Approve call in: <strong>{minutes}:{seconds}</strong>
      </div>
      <div className="text-xs text-gray-500">Next scheduled call: {format(nextCallAt, "dd MMM yyyy HH:mm:ss")} (server time)</div>
    </div>
  );
}
...
export default function ClientBasedFinOpsTaskManager() {
  ...
  const approveMutation = useMutation({
    mutationFn: async ({ subtaskId, approverName }: { subtaskId: number; approverName: string }) => {
        ...
        <div key={`task-${task.id}`} className="p-4 border rounded-md bg-white">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">Client: {task.client_name || 'Unknown Client'}</p>
            </div>
            <div className="text-sm text-gray-500">Last run: {task.last_run || 'N/A'}</div>
          </div>
          ...
          <div className="mt-3 space-y-2">
            {Array.isArray(task.subtasks) && task.subtasks.map((subtask: any) => {
              ...
              const reportingList: string[] = Array.isArray(task.reporting_managers)
                ? task.reporting_managers
                : (typeof task.reporting_managers === 'string' && task.reporting_managers ? JSON.parse(task.reporting_managers) : []);

              const isReporting = reportingList.some((r) => normalize(r) === normalize(user?.name));

              return (
                <div key={`sub-${subtask.id}`} className="p-3 border rounded-md bg-gray-50">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-medium">{subtask.name}</div>
                      <div className="text-xs text-gray-500">Status: {subtask.status}</div>
                    </div>
                    <div className="flex items-center gap-2">
                      {subtask.status === 'completed' && !isApproved && isReporting && (
                        <div className="flex items-center gap-3">
                          <Button
                            onClick={async () => {
                              try {
                                const approverName = user?.name || "";
                                await approveMutation.mutateAsync({ subtaskId: Number(subtask.id), approverName });
                              } catch (e) {
                  ...
                  {subtask.status === 'completed' && !isApproved && isReporting && (
                    <ApprovalTimer taskId={task.id} subtaskId={Number(subtask.id)} />
                  )}

                  {/* If approved, show approval info */}
                  {isApproved && (
                    <div className="mt-2 text-sm text-green-700">Approved by {(subtask as any).approved_by} at {(subtask as any).approved_at || ''}</div>
                  )}
                </div>
              );
            })}
